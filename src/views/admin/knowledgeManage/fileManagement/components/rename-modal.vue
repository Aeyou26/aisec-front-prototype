...
<template>
  <a-modal v-model:visible="visible" title="重命名" title-align="start" :width="680" :mask-closable="false" @before-open="onBeforeOpen" @cancel="onClose">
    <a-form ref="formRef" layout="vertical" :model="form" auto-label-width>
      <a-form-item label="名称" field="address" :rules="[{ required: true, message: '请输入地址' }]">
        <a-input v-model="form.address" placeholder="请输入名称" />
      </a-form-item>
    </a-form>
    <template #footer>
      <a-space>
