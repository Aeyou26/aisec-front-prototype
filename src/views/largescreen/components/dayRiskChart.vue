<template>
  <div class="chart-main">
    <div class="chart-title">
      <!-- 报警概况 -->
      整体风险漏洞趋势
      <div class="chart-subtitle">(近7日)</div>
    </div>
    <div class="relative w-400">
      <div v-show="hasData" id="overview" class="chart-item w-400 h-250"></div>
  ...
  request
    ...
    .then((res: any) => {
      if (res && res.data && Object.keys(res.data).length > 0) {
        ...
        // 输出结果
        console.log('月份:', day)
        console.log('高危:', highRisk)
        console.log('中危:', mediumRisk)
        console.log('低危:', lowRisk)

        // 将结果赋值给 ECharts 的 option
        option.xAxis.data = day // 设置 x 轴数据
        ...
        if (dom) {
          ...
          overviewDom.value = document.getElementById('overview') as HTMLElement
